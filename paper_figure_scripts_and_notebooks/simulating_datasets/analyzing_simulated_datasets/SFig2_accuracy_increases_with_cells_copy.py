...
parser.add_argument('--path_to_code', type=str, default='/Users/Daan/Documents/postdoc/bonsai-development',
                    help='Path to bonsai-development folder.')

args = parser.parse_args()
sys.path.append(args.path_to_code)
...
fig, axs = plt.subplots(nrows=1, ncols=len(ns_cells_per_clst), figsize=(10, 4), sharey=True)
if len(ns_cells_per_clst) == 1:
    axs = [axs]
dist_objcts = []
# bonsai_dist_objcts = []
# umap_dist_objcts = []
...
for ind_dim, n_cells_per_clst in enumerate(ns_cells_per_clst):
    n_cells = n_clsts * n_cells_per_clst
    subset_cells = np.arange(0, n_cells, n_cells_per_clst, dtype=int)
    args.input_simulated_dataset = 'data/simulated_datasets/simulated_equidistant/simulate_equidistant_{}_clsts_{}_cells_{}_dims_random_times_no_umi_counts{}_seed_84'.format(
        n_clsts, n_cells, num_dims, add_noise)
    args.bonsai_results = 'results/simulated_datasets/simulated_equidistant/simulate_equidistant_{}_clsts_{}_cells_{}_dims_random_times_no_umi_counts{}_seed_84/final_bonsai_zscore1.0'.format(
        n_clsts, n_cells, num_dims, add_noise)
    args.results_folder = 'useful_scripts_not_bonsai/simulating_datasets/analyzing_simulated_datasets/results/simulated_datasets/simulated_equidistant/simulate_equidistant_{}_clsts_{}_cells_{}_dims_random_times_no_umi_counts{}_seed_84'.format(
        n_clsts, n_cells, num_dims, add_noise)
    Path(os.path.join(args.results_folder, 'intermediate_files')).mkdir(parents=True, exist_ok=True)

    # Get cell-ids
    if (not RECALCULATE) and os.path.exists(os.path.join(args.results_folder, 'intermediate_files', 'cellID.txt')):
        cell_ids = []
        with open(os.path.join(args.results_folder, 'intermediate_files', 'cellID.txt'), 'r') as file:
    ...
    if RECALCULATE or (
            not os.path.exists(os.path.join(args.results_folder, 'intermediate_files', 'delta_true_pdists.npy'))):
        ...
        np.save(os.path.join(args.results_folder, 'intermediate_files', 'delta_true_pdists.npy'), true_dists,
                allow_pickle=False)

    # Calculate pairwise distances for Bonsai.
    if RECALCULATE or (
            not os.path.exists(os.path.join(args.results_folder, 'intermediate_files', 'bonsai_pdists.npy'))):
        ...
        np.save(os.path.join(args.results_folder, 'intermediate_files', 'bonsai_pdists.npy'), bonsai_dists,
                allow_pickle=False)

    if DO_OTHER_TOOLS:
        # Perform PCA-projection.
        all_pca_files = [os.path.basename(filepath) for filepath in
                         Path(os.path.join(args.results_folder, 'intermediate_files')).glob('pca_*.npy')]
        ...
        if (not RECALCULATE) and len(all_pca_files):
            ...
            for pca_file in all_pca_files:
                ...
                pca_projected[n_comps] = np.load(os.path.join(args.results_folder, 'intermediate_files', pca_file),
                                                 allow_pickle=False)
        else:
            ...
            for n_comps, pca_proj in pca_projected.items():
                np.save(os.path.join(args.results_folder, 'intermediate_files', 'pca_{}.npy'.format(n_comps)), pca_proj,
                        allow_pickle=False)

        # Perform UMAP.
        all_umap_files = [os.path.basename(filepath) for filepath in
                          Path(os.path.join(args.results_folder, 'intermediate_files')).glob('umap_*.npy')]
        ...
        if (not RECALCULATE) and len(all_umap_files):
            ...
            for umap_file in all_umap_files:
                ...
                umap_projected[n_comps] = np.load(os.path.join(args.results_folder, 'intermediate_files', umap_file),
                                                  allow_pickle=False)
        else:
            umap_projected = {}
            for n_comps, pca_proj in pca_projected.items():
                ...
                np.save(os.path.join(args.results_folder, 'intermediate_files', 'umap_{}.npy'.format(n_comps)),
        ...
        all_pca_dist_files = [os.path.basename(filepath) for filepath in
                              Path(os.path.join(args.results_folder, 'intermediate_files')).glob('pca_*_pdists.npy')]
        if RECALCULATE or (not len(all_pca_dist_files)):
            for n_comps, pca_proj in pca_projected.items():
                if n_comps != 2:
                    continue
                pca_dists = distance.pdist(pca_proj.T, metric='sqeuclidean') / 2
                np.save(os.path.join(args.results_folder, 'intermediate_files', 'pca_{}_pdists.npy'.format(n_comps)),
                        pca_dists,
                        allow_pickle=False)

        all_umap_dist_files = [os.path.basename(filepath) for filepath in
                               Path(os.path.join(args.results_folder, 'intermediate_files')).glob('umap_*_pdists.npy')]
        if RECALCULATE or (not len(all_umap_dist_files)):
            for n_comps, umap_proj in umap_projected.items():
                ...
                np.save(os.path.join(args.results_folder, 'intermediate_files', 'umap_{}_pdists.npy'.format(n_comps)),
                        umap_dists, allow_pickle=False)

    if DO_OTHER_TOOLS:
        alldistfiles = list(Path(os.path.join(args.results_folder, 'intermediate_files')).glob('*_pdists.npy'))
    else:
        alldistfiles = list(Path(os.path.join(args.results_folder, 'intermediate_files')).glob('*bonsai*_pdists.npy'))
        alldistfiles += list(Path(os.path.join(args.results_folder, 'intermediate_files')).glob('*true*_pdists.npy'))
...
ax = axs_bp
bplot = ax.boxplot(pearsonRSqs, whis=(5, 95), labels=dataset_names, patch_artist=True, flierprops=flierprops,
                   medianprops=medianprops, boxprops=boxprops)
# fill with colors
for ind_patch, patch in enumerate(bplot['boxes']):
    patch.set_facecolor(color=plot_colors[ind_patch])
    if ind_patch % 3 == 1:
        ax.text(ind_patch + 1, .9, "{}-cell clusters".format(ns_cells_per_clst[ind_patch // 3]), ha='center', va='bottom', transform=ax.get_xaxis_transform(), fontsize=14)
...
base_folder = '/Users/Daan/Documents/postdoc/bonsai-development/data/simulated_datasets/simulated_equidistant'

plt.savefig(os.path.join(base_folder, "SI_tree_better_at_more_cells_new.svg"))
plt.savefig(os.path.join(base_folder, "SI_tree_better_at_more_cells_new.png"))
