...
```
* Replace <x.y.z> in the line below with your choice and then run the command:
```
module purge
...
`bonsai-development/bonsai/create_config_file.py`. One can for example run it by specifying only the path to where
you want to store this YAML-file. From the `bonsai-development` directory, run:
```
...
From the main directory "bonsai-development" one can for example run (note that all paths should be either absolute or 
relative to the "bonsai-development"-folder):
```
...
```
*See the Section [Possible run configurations](https://github.com/dhdegroot/bonsai-development/blob/main/README.md#possible-run-configurations) for information on the different configurations that you can change. **For first usage, one only needs to customize the configurations `dataset`, `data_folder`, `filenames_data`, `results_folder`, `input_is_sanity_output`. The other configurations are only required for advanced usage.***

### Running Bonsai on a single core
Once you have prepared the YAML-file, you only need to start the *Bonsai* run. 
It now takes only two arguments. The argument `--config_filepath` is strictly required and should contain the path to the configuration-file (either relative to the "bonsai-development"-directory, or absolute. The `--step`-argument is optional, and allows us to cut the calculation up into bricks that have different computational resource requirements, see [Example 2](https://github.com/dhdegroot/bonsai-development/blob/main/README.md#example-2-running-bonsai-on-an-hpc-cluster) below, for an example. Setting `--step all` will just run the full algorithm at once. From the "bonsai-development"-directory, one thus runs:
```
...
* **a subdirectory starting with `final_bonsai_`.** This directory will contain the final state of the tree stored in two ways. 
  ...
  - Second, the files `edgeInfo.txt`, `vertInfo.txt`, `metadata.json` together contain a more human-readable form of storing the tree. These files are necessary for visualizing the tree using the `vis_bonsai`-app.
...
First, make sure that you have the required packages installed, we need a few packages more than for the *Bonsai*-reconstruction, and also *Shiny* works better with *Python 3.9*. Therefore, we create a new conda environment for the Bonsai-visualization. (If you created this environment before, you can easily switch with `conda activate vis_bonsai`).

* Make a new conda environment:
```
conda create --name vis_bonsai python=3.9 -y
conda activate vis_bonsai
```
* Install the required packages:
```
pip install -r requirements_vis_bonsai.txt
```

### Preprocessing for the visualization
Before running the app, we must first do a preprocessing run, using (from "bonsai-development")
```
python3 vis_bonsai/vis_bonsai_preprocess.py \
...
Now you can view your results in *Bonsai*'s Shiny app by running (from the "bonsai-development"-directory)
```
python3 vis_bonsai/run_shiny_bonsai.py \
--results_folder <BONSAI_RESULTS_FOLDER> \
...
To test if *Bonsai* is set-up correctly, let's run it on a very small dataset of 64 cells. The output from *Sanity* for this dataset is stored in `bonsai-development/examples/example_data/simulated_binary_6_gens_samplingNoise`. 
